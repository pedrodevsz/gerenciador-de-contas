...
export function FixedRevenuesTable() {
    const fixRevenues = useRevenueStore((s) => s.fixRevenues)
    ...
    return (
        <Card className="w-full overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <CardHeader className="px-4 pb-0 sm:px-6">
                <CardTitle className="text-foreground text-base md:text-lg font-semibold">
                    Ganhos Fixos
                </CardTitle>
            </CardHeader>

            <CardContent className="px-4 py-4 sm:px-6 sm:py-5">
                {fixRevenues.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-border/80 px-4 py-8 text-center text-sm text-muted-foreground sm:text-base">
                        Nenhum ganho fixo registrado
                    </div>
                ) : (
                    <>
                        <div className="space-y-3 md:hidden">
                            {fixRevenues.map((item) => (
                                <div
                                    key={item.id}
                                ...
                                <TableBody>
                                    {fixRevenues.map((item) => (
                                        <TableRow key={item.id} className="border-border transition-colors hover:bg-muted/50">
                                            ...
                                            <TableCell className="px-4 py-4 text-right font-bold text-emerald-600 dark:text-emerald-500 lg:px-6">
                                                {formatCurrency(item.value)}
                                            </TableCell>
                                        </TableRow>
                                    ))}
